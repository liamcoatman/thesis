...
In a further 71 objects [\ion{O}{III}] is very weak or the spectrum is very low S/N.
In these quasars we vary the normalisation of a fixed [\ion{O}{III}] template (see below).   
...
In many objects [\ion{O}{III}] is undetected, or is very low S/N. 
In these cases we do not attempt to measure the width of the [\ion{O}{III}] emission, but instead fit a fixed [\ion{O}{III}] template, in order to measure the EQW. 
...
This object was studied by \citet{shen16a}, but residual \ion{Fe}{II} was mistakenly identified as [\ion{O}{III}], resulting in a catastrophically wrong systemic redshift estimate. 
...
We estimate that this has a significant effect in 58 quasars in our sample. 
The majority of these (41) are flagged as having low EQW [\ion{O}{III}]. 
This is expected from the well-known anti-correlation between the strengths of \ion{Fe}{II} and [\ion{O}{III}] (Eigenvector 1). 
Therefore, our failure to adequetly remove the \ion{Fe}{II} emission does not really affect our ability to measure the [\ion{O}{III}] properties. 
The biggest problem is if residual \ion{Fe}{II} emission is misidentified as [\ion{O}{III}] \citep[e.g.][]{netzer04}. 

In 3 objects, there is a plateau-like emission. 
It's difficult to know what fraction of this emission is \ion{Fe}{II}, and what fraction is [\ion{O}{III}]. 

\begin{figure}
    \centering
    \includegraphics[width=0.8\columnwidth]{figures/chapter04/compare_gaussian_ica_QSO538.pdf} 
...
This doesn't mean we are having to exclude all of these because of the \ion{Fe}{II} - [\ion{O}{III}] is missing in most anyway. 

\subsubsection{Low EQW [\ion{O}{III}]}

If the EQW is too low then the shape of the [\ion{O}{III}] emission cannot be measured reliably, regardless of the S/N of the spectra.  
We therefore impose an EQW cut of 8\AA\, on our derived parameters. 

Note that if very low EQW or S/N I fit with fixed model just to get an estimate of the EQW. 

\begin{figure}
    \centering
    \includegraphics[width=0.6\textwidth]{figures/chapter04/oiii_eqw_hist.pdf} 
    \caption{Anything to the left of the line [\ion{O}{III}] is too weak to derive paramters from Gaussian fits. Excludes 120 objects.}     
...
Specifically, the velocity measures such as $w_{80}$. 
