...
\ion{C}{IV} emission line properties (used to measure outflows in the BLR) are taken directly from Chapter~\ref{ch:bhmass}. 
...
\ion{Fe}{II} emission is generally strong in the vicinity of [\ion{O}{III}]. 
Therefore, before [\ion{O}{III}] is modelled, we first model and subtract the continuum and \ion{Fe}{II} emission using the procedure described in Chapter~\ref{ch:bhmass}. 

We encountered 24 objects for which \ion{Fe}{II} emission appears to be present in the spectrum even after the subtraction procedure. 
In these objects the relative strengths of the \ion{Fe}{II} lines appear to differ significantly from those of I Zw 1 on which the \ion{Fe}{II} template we use is based. 
...
Our analysis suggests that emission which was modelled by \citet{shen16a} as [\ion{O}{III}] is more likely to be \ion{Fe}{II} which is blue-ward of the [\ion{O}{III}] laboratory wavelength. 
Because the derived [\ion{O}{III}] emission properties can be strongly biased in objects so-affected, these objects are flagged and are excluded from our analysis in the remainder of this chapter. 

\subsection{Modelling \hbns/[\ion{O}{III}]}

The \hb and [\ion{O}{III}] emission is fit, again using the procedure described in Chapter~\ref{ch:bhmass}. 
...
Therefore, when the [\ion{O}{III}] line shape is analysed in later sections, these objects with EQW $<$ 8\AA will be excluded.
...
\begin{itemize}
  ...
  \item[31-32] 4434 - 4684 \AA rest-frame EQW of \ion{Fe}{II}, and its error, in \AA. % Not the same as used in EV1 plots 
