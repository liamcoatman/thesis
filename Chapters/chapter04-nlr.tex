...
The relative balance between the core and wing components varies significantly from object to object, and governs the width and asymmetry of the overall [\ion{O}{III}] emission profile \citep[e.g. luminosity;][]{shen14}. 
...
The rest-frame transformation is only required to be accurate to within $\sim1000$\,\kms\, of the systemic redshift for our fitting procedure to function. 
...
\begin{figure}
    ...
    \caption[{Spectra of the $24$ objects for which significant \ion{Fe}{II} emission is still visible following our \ion{Fe}{II}-subtraction procedure.}]{Spectra of the $24$ objects for which significant \ion{Fe}{II} emission is still visible following our \ion{Fe}{II}-subtraction procedure. Spectra have been smoothed via convolution with a $100$\,\kms\, Gaussian kernel. The vertical lines indicate the expected positions of the [\ion{O}{III}] doublet (which is generally very weak in these objects) with the systemic redshift defined using the peak of the broad \hb emission.}     
    \label{fig:bad_fe}
\end{figure}
...
In this section, we compare our objects to a sample of SDSS quasars in order to investigate the luminosity and redshift dependence of key [\ion{O}{III}] parameters. 
...
Considering only the objects for which [\ion{O}{III}] was detected with ${\mathrm EQW} > 1$\,\AA\, we observe a modest decrease in the [\ion{O}{III}] EQW as the luminosity increases (from $18$\,\AA\, at $L_{\mathrm Bol}=10^{45.25}$\,\ergs to $12$\,\AA\, at $L_{\mathrm Bol}=10^{47.75}$\,\ergs. 
However, the fraction of objects with [\ion{O}{III}] ${\mathrm EQW} < 1$\,\AA is $10$ per cent in our sample, compared to just one per cent of the SDSS sample.
This is explored further in Section~XX. 
...
The size of the NLR is predicted to scale with the square root of the luminosity of the source of ionising photons \citep[e.g.][]{netzer90} and low-luminosity Seyfert galaxies appear to obey this relation \citep[e.g.][]{bennert02}. 
...
In Figure~\ref{fig:lum_w80} we show that the [\ion{O}{III}] velocity-width is also correlated with the quasar bolometric luminosity.
...
When [\ion{O}{III}] is strong, the \ion{C}{IV} blueshift is measured relative to its peak. 
Otherwise, the \ion{C}{IV} blueshifted is measured relative to \hb or \ha.  
In Section~XX we found that redshifts measured from all three of these lines are consistent to within $\sim300$\,\kms, which is small in comparison to the dynamic range in \ion{C}{IV} blueshifts we see in Figure~\ref{fig:ev1}.
Also shown are the \ion{C}{IV} line parameters of $32\,157$ SDSS DR$7$ quasars at redshifts $1.6 < z < 3.0$. 
The \ion{C}{IV} emission line properties of these objects are described in Section~XX, and \ion{C}{IV} blueshifts are measured using Allen \& Hewett (2017, in preparation) systemic redshifts. 
...
Qualitatively, the distribution of objects in a two-dimensional projection of the EV$1$ parameter space (Figure~\ref{fig:ev1_lowz}) is very similar to the distribution of objects in the \ion{C}{IV} blueshift-EQW parameter space (Figure~\ref{fig:ev1}).
...
This establishes a connection between EV$1$ and \ion{C}{IV} parameters \citep[e.g.][]{sulentic07}. 
However, while objects with large \ion{C}{IV} blueshifts had narrow \ha emission-lines, many objects with small \ion{C}{IV} blueshifts did too. 
In contrast, Figure~\ref{fig:ev1} demonstrates that the [\ion{O}{III}] provides a less degenerate mapping between the EV$1$ and \ion{C}{IV} parameter spaces.
...
Blueshifts in the \ion{C}{IV} emission that can reach many thousands of \kms\, is strong evidence that quasars are capable of driving fast outflows, at least over the sub-parsec scale of the BLR.
One possibility is that the NLR gas is swept away on relatively short time-scales by quasar outflows. 
...
It would take $10^6$ years for an outflow travelling at $3000$\,\kms\, to reach $3$\,kilo-parsec. 
Dynamical time-scales for the impact of fast moving outflows even on large NLRs are very short, which could be at least part of the explanation for the increasing fraction of luminous quasars without NLRs producing [\ion{O}{III}] emission. (\ion{C}{IV} blueshift increases with luminosity)

\subsection{Connections between NLR and BLR outflows}
...
In Figure~\ref{fig:oiii_civ_blueshifts} we show the [\ion{O}{III}] blueshift as a function of the \ion{C}{IV} blueshift, for the objects where [\ion{O}{III}] is detected with EQW$>8$\,\AA. 
...
Alternatively, in Section~XX we reported an anti-correlation between the [\ion{O}{III}] EQW and asymmetry. 
We found that the [\ion{O}{III}] emission becomes broader and more blueshifted as the EQW decreases.
Given the strong anti-correlation between the [\ion{O}{III}] EQW and the \ion{C}{IV} blueshift. 
A similar correlation between the [\ion{O}{III}] EQW and the strength of \ion{Fe}{II} was reported by \citet{shen14}. 
\citet{shen14} also showed that the amplitude of the core [\ion{O}{III}] emission decreases faster than the wing component. 
...
Therefore, the correlation observed in Figure~\ref{fig:oiii_civ_blueshifts} could be due to the core component dropping faster, rather than the wing component getting stronger. 
