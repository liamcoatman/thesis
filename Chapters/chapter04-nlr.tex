...
Blue-asymmetric structure is generally associated with outflows, and so this suggests that outflows are prevalent in the NLRs of these objects. 
...
Lifetimes of luminous quasars at the these redshifts may be $10^7$ years \citep[e.g.][]{martini01}. 
...
The winds could collide  with the interstellar medium of the surrounding host and shock and accelerate it to produce a galaxy-wide wind \citep[e.g.][]{king11,faucher12}. 
...
In light of this, the high fraction of radio-loud objects amongst the quasars with very broad and blueshifted [\ion{O}{III}] emission is interesting. 
Relativistic jets are known to accelerate gas to velocities of up to a few thousand \kms\, \citep[e.g.][]{nesvadba06,nesvadba08} and so the disturbed gas kinematics could be induced by radio emission. 
...
Our calculations are based on the model of \citet{cano-diaz12}. 

Following \citet{cano-diaz12}, the mass in the ionised outflow is given by 

\begingroup\makeatletter\def\f@size{11}\check@mathfonts
\begin{eqnarray}
M \simeq 5.33 \times 10^7 \left( \frac{C}{10^{\mathrm[O/H] - [O/H]_\odot}} \right) \left( \frac{L([OIII])}{10^{44}\, {\mathrm erg}\,{\mathrm s}^{-1}}\right) \\ \times \left\langle \frac{n_e}{10^3\, {\mathrm cm}^{-3}} \right\rangle^{-1} \, M_\odot
\end{eqnarray}
\endgroup

\noindent where $L([OIII])$ is the luminosity of [\ion{O}{III}] emitted in the outflow (in units of $10^{44}$\,\ergs), $\langle n_e \rangle$ is the electron density in the outflowing gas (in units of $10^3$\,cm$^{-3}$), $10^{[{\mathrm O/H}]}$ is metallicity (in units of Solar metallicity), $C$ ($=\langle n_e \rangle ^2 / \langle n_e^2\rangle$) is the condensation factor (assumed to be $\simeq1$). 
Assuming a conical outflow with uniformly distributed clouds out to a radius $R$ with a constant outflow velocity, the outflow rate of the gas is given by: 
...
For the outflow radius we use $3$\,kilo-parsec \citep[e.g.][]{cano-diaz12,carniani15,brusa16,zakamska16}. 

With these values, we calculate an outflow rate of about $200\,M_\odot\,{\mathrm yr}^{-1}$ and a kinetic power of about $3\times10^{44}$\,\ergs. 
This is consistent with previous studies at similar luminosities and redshifts (e.g. )
The kinetic power is about $10^{-3}$ times the bolometric luminosity ($3.8\times10^{46}$\,\ergs).  
However, if the ionized outflow is accompanied by a neutral/molecular outflow an order of magnitude more massive then the kinetic power is also likely to be an order of magnitude higher, i.e. about $1$ per cent of the bolometric luminosity \citep{cano-diaz12}. 
Recent AGN feedback models (e.g. King 2010; Zubovas \& King 2012; Lapi et al. 2014) predict a coupling efficiency between AGN-driven outflows and AGN power of $\sim5$ per cent which is needed to explain the $M_{\mathrm BH}-\sigma$ relation observed in local galaxies.
...
I'm assuming 100 electron density so 


Going to only do this for the objects fit with two Gaussians
Computing L[O III] from the integrated flux in the broad component only. 

Need to make sure my luminosities make sense for this! 

L of broader component
Make sure its blueshifted! 

Seems stupid decomposing - what was the other thing someone used w80/1.2 or something? 
...
A comprehensive description of these as-
sumptions and their impact on the determination of the outflow parameters is presented in Kakkad et al. (2016), which provides
further details.
