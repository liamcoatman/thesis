...
However, our systemic redshift estimates that use the peak of the \hb emission (Section~\ref{sec:redshifts}) will not be affected. 
...
The peak flux ratio of the [\ion{O}{III}] 4960\,\AA\, and 5008\AA\, components are fixed at the expected 1:3 ratio and the width and velocity offsets are set to be equal\footnote{For J003136+003421, a significantly better fit ($\Delta \chi^2_{\nu} \sim 25\%$) is obtained when the peak flux ratio constraint relaxed; the peak ratio of the best-fitting model is 1:2.13.}.

In 62 objects with very weak [\ion{O}{III}] (mean EQW$\sim2$\AA) we found that the Gaussian model has a tendency to fit features to the noise. 
...
The [\ion{O}{III}] models, and the numbers of quasars each model is applied to, are summarised in Table~\ref{tab:oiiimod}.
...
\todo{Comment on why chi-squared is so big}
...
\todo{This negelects the uncertainty on the fit to the continuum/iron/other emission. Just use the uncertainty from my parameter uncertainties, but be careful with whether I am using template or not. Then use upper limit if the EQW < 3sigma (say)}
...
Line-width measures are corrected for instrumental broadening \todo{Paul: what about the 200 \kms (can't read)} by subtracting the resolution of the spectrograph (Table~\ref{tab:specres}) in quadrature.
...
\todo{Paul: in the same sense as Heckman 1981 and HW10}
...
\begin{figure}
    ...
    \caption[{Probability density distributions of the [\ion{O}{III}] parameters EQW (a), $w_{80}$ (b) and asymmetry $R$ (c).}]{Probability density distributions of the [\ion{O}{III}] parameters EQW (a), $w_{80}$ (b) and asymmetry $R$ (c).}     
...
\todoinline{Explain why Baldwin effect - gas photoinoised by accretion disc. as you have more ionising photons the NLR grows, but then there is no more gas to inoise and the size plateus.}
\todoinline{What is Netzer interpretation?}
\todoinline{Plot the EQW strength as a function of luminosity (Show histograms?)}

\todoinline{
Theoretical suggestions that the NLR size should scale with the square root of L\_Hb (e.g. Netzer 1990). 
Similar to the R-L relation scaling fro the BLR. 
This seems to be confirmed in low luminosity AGN. 
Howver, it raises severe problems for high luminosity quasars. 
Any such scaling leads to sizes that are larger than galactic sizes. 
Typical NLRs (those similar in properties to ones in Seyferts) cannot last very long in high luminosity quasars, because they have galactic dimensions and are likely to excape the system. 
This means that when we have strong [\ion{O}{III}] it may be of a different origin and physical properties.
In particular the gas densities must be orders of magnitude larger. 
A possible origin of this high density gas in kpc scale nuclear regions is violent star formation. 
}
...
\todo{histogram RFeii? do we see typically stronger iron suggesting higher accretion rates?} 
...
The [\ion{O}{III}] velocity widths of the \citet{zakamska16} objects are extreme in relation to our sample, matched in 5 $\mu$m luminosity.
\todo{A couple of paragraphs above I same they are `similar' so need to be consistent} 
...
Although the ICA analysis is not based on any physics,  there appears to be a direct correspondence between the individual non-negative components and the different emission features which contribute to the spectra (Fig.~\ref{fig:mfica_components}). 
...
\begin{figure}
    ...
    \caption[{[\ion{O}{III}] emission in J002952+020607.}]{[\ion{O}{III}] emission in J002952+020607. The data is shown in blue, and the ICA spectrum in grey. The first three ICA components have been subtracted from both the ICA composite and the data. The black curve shows the reconstructed [\ion{O}{III}]\l5008 profile.}     
