...
\ac{AGN} are very efficient at driving outflows; X-ray and \ac{UV} spectroscopy reveal high velocity outflows to be nearly ubiquituous on sub-parsec scales in high accretion rate quasars.
In recent years, a huge amount of resources have been devoted to searching for observational evidence of galaxy-wide, \ac{AGN}-driven ouflows, which could provide the feedback mechanism nessecary to quench star formation in massive galaxies. 
...
One particularly successful technique has been observations of forbiden emission lines, which trace warm (T$\sim$$10^4$K) ionised gas in the \ac{NLR}. 
...
This has provided constraints on the prevalence of ionised outflows and, by measuring outflow properties as a function of \ac{AGN} properties, on the drivers of these outflows.  
...
This could suggets that \ac{AGN} efficiency in driving galaxy-wide outflows increases with \ac{AGN} luminosity. 
...
The large sample size will help to put these observations in context of the \ac{AGN} population as a whole.
...
The template is derived by running our fitting routine on a very high \ac{S/N} composite spectra of low redshift \ac{AGN}. 
...
Briefly, the SED-model was fit, with the normalisation and E(B-V) as free varibales, to optical/infrared m\ac{AGN}itudes, or SDSS/BOSS spectra (check order I do this.)
...
\begin{figure}
    ...
    \caption{Change in the [\ion{O}{III}] line parameter $w_{80}$ as the \ac{S/N} is artifically varied from 3-50 for two different objects. At each \ac{S/N}, our line-fitting procedure is run on 100 mock spectra, and the points and errorbars indicate the 50th percentile and 16-84 percentile range respectively. The spectra is flagged if $|\Delta w_{80}|>0.1$ at the \ac{S/N} of the spectra (shown by the vertical line).}
    \label{fig:snr_test}
\end{figure}

In this section we flag objects with poor spectral \ac{S/N}. 
A single \ac{S/N} cut is not adequate: for a given \ac{S/N}, it is much easier to measure the properties of a strong line than a weaker one. 
Our approach is therefore as follow:

\begin{enumerate}
\item For each object use the best-fitting parametric model as a high \ac{S/N} representation of the spectra. 
\item Scale the error spectrum so that the \ac{S/N} (measured in the continuum and quoted per pixel) is \{2.5, 5, 7.5, 10, 15, 20, 50\}.
\item At each \ac{S/N}, generate 100 mock spectra by randomly drawing the flux in each pixel from a Normal distribution with mean $\mu$ equal to the model flux and width $\sigma$ equal to the scaled error. 
...
\item A low \ac{S/N} flag is assigned to the object if the value of $w_{80}$ changes by more than 10 per cent from the highest \ac{S/N} representation to the \ac{S/N} of the data. 
\end{enumerate}
...
The first object would pass our \ac{S/N} cut, whereas the latter would fail. 

\subsection{Model Two: Independent Component Analysis}

\ac{ICA} is a blind source separatation tecnique for separating a signal in to linearly mixed statistically independent subcomponents. 
...
Each of the \ac{ICA} components is then divided by the power-law. 
...
Because there are so few objects with very broad [\ion{O}{III}] in the low-redshift sample, the \ac{ICA} reconstuction fails to reproduce the broadest [\ion{O}{III}] profiles in our sample. 
...
This is bourne out by the fact that there are quite large differences between, for example, $v_{10}$ measured from the Gaussian fit and $v_{10}$ measured from the \ac{ICA} fit. 
...
This has led to the so-called \ac{EV1} correlates \citep{boroson92}. 
...
The \ion{C}{IV} blueshift and \ac{EQW} is a di\ac{AGN}ostic that similarly spans the diversity of broad emission line properties in high redshift quasars (dominated by a virialized component at one extreme and a wind driven component at the other) \citep{richards11,sulentic07}. 
The similarity of the \ion{C}{IV} \ac{EQW}-blueshift parameter space at high redshift to \ac{EV1} parameter space at low redshift suggetss that these trends are connected. 
...
Can do something similar with \ac{ICA} weights, but I'm not sure what the best question to ask is. 
Could make a linear model of the \ac{ICA} component weights and fit to the \ion{C}{IV} blueshift? 

\begin{figure}
    \includegraphics[width=\columnwidth]{figures/chapter04/ev1.pdf} 
    \caption{The high-$z$ \ac{EV1} parameter space of \ion{C}{IV} blueshift and \ac{EQW}. Our sample is shown with points, and quasars from the full \ac{SDSS} catalogue are shown with grey contours. The [\ion{O}{III}] EQW varies systematically with position in the \ion{C}{IV} blueshift-\ac{EQW} parameter space (a) but \hb FWHM shows signficantly less systematic variation (b)..      
    \label{fig:ev1}
\end{figure}



Because of this diversity, it is the dominant variable in the set of correlations making up \ac{EV1}, which is believed to be linked to certain fundamental parameters of the accretion process. 
In Figure~\ref{fig:ev1} we show the [\ion{O}{III}] \ac{EQW} as a function of the \hb FWHM and the optical \ion{Fe}{II} strength. 
...
These parameters form part of \ac{EV1}, the first eigenvector in a principal component analysis which originated from the work of \citet{boroson92}.
...
Same as \citet{shen16a}, we confirm that the \ac{EV1} correlations hold at high luminosities/redshifts. 
...
Zhang et al. 2011: Blueshift of [\ion{O}{III}] correlates significantly with the \ac{EQW} of the core. 
...
Consistent with the core coming from the canonical extended \ac{NLR} where the gas is dominate by gravity of the bulge while the wing arises in an outflow. 
And \ion{C}{IV} explained by wind. Suggests intimate connection between \ac{BLR} and \ac{NLR}.

\subsubsection{Comparison of low-$z$ and high-$z$ samples}

In this section we compare the \ac{ICA} component weights in our low-$z$ and high-$z$ samples. 
...
\begin{figure}
    ...
    \caption{[\ion{O}{III}] strength decreases as the \ion{C}{IV} blueshift increases. See similar thing if I use [\ion{O}{III}] \ac{EQW} instead. Only showing the core components here. The \ion{C}{IV} blueshift is now measured relative to the NIR \ac{ICA} redshift. I think this is trend is mostly being driven by the Eigenvector 1 correlations: as the blueshift increases the \ion{Fe}{II} strength increases and the [\ion{O}{III}] strength decreases. Doesn't appear to be driven by the luminositiy. Is this tighter than \ac{EV1} trend shown with Fe/OIII strength by other authors? Is the \ac{AGN} \ac{NLR} absent in objects where outflows have reached
...
\begin{figure}
    ...
    \caption{\ac{ICA} median weights as a functino of the CIV blueshift.}     
...
At high redshifts we have the \ion{C}{IV} blueshift and \ac{EQW}. 
...
In this section we do a comparison of systemic redshift estimates from [\ion{O}{III}], broad \hb and \hans, and from fitting the \ac{ICA} component weights. 
...
\todo{Need to recalculat \ac{ICA} redshifts since shift to fit in log space}. 
Benefit of \ac{ICA} is that it works regarless of the [\ion{O}{III}] strength. 
...
Comparing NIR \ac{ICA} to [\ion{O}{III}] for the [\ion{O}{III}] with high \ac{S/N} I find small (few hundred \kms) scatter. 
...
We then perform an error-weighted linear least-squares regression with an [\ion{O}{III}] template derived from a fit to a very high \ac{S/N} low redshift SDSS composite spectra. 
...
On the other hand, there are many weak or no [\ion{O}{III}] emitters at high luminosity that could give the impression that the line \ac{EQW} decreases with increasing source luminoisity. 
