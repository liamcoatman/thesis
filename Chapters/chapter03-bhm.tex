...
Emission-line velocity widths are derived from the FWHM of the lines but we also compute the line dispersion as some authors have claimed this provides a better estimate of the virial velocity \citep{denney13}.
...
Individual continuum- and \ion{Fe}{II}- (\hb only) subtracted spectra were first de-redshifted to the quasar rest-frame, and then interpolated on to a common wavelength grid with a $1$\,\AA\, resolution.
...
In the above relations, $\text{FWHM}(\text{C}\,\textsc{iv}, \text{Corr.})$ is the FWHM of \ha or \hb obtained from the \ion{C}{IV} FWHM and blueshift, using the fitted relation.
...
Equations~\ref{eq:hafwhm} and \ref{eq:masscorrection} together provide a good estimate of the virial BH mass given the FWHM and blueshift of \ion{C}{IV}, together with the continuum luminosity at $1350$\,\AA.
...
However, there is a suggestion that the model may differ systematically from the data at low blueshifts when \ion{C}{IV} is compared to \hb (see Figure~\ref{fig:correction_hb}).
If a zero-bias mass correction is desired, one could derive such a correction by directly fitting M$_{\text{BH}}$(\ion{C}{IV})/M$_{\text{BH}}$(\hbns) vs \ion{C}{IV} blueshift.
...
\citet{richards02} found that quasars with large \ion{C}{IV} blueshifts have weak \ion{He}{II}, which is evidence for weak soft X-ray continuum emission \citep{leighly04}.
