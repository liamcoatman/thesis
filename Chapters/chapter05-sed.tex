...
The template $F_{\lambda, {\rm Gal}}$ is scaled by a constant factor $C$, which is a free paramter in our optimisation procedure.  
We define a new parameter, $\eta$, the fractional contribution from the host galaxy to the total flux in the interval 4000 and 5000\AA for a quasar at redshift $z_{\rm nrm}$: 
...
where $F_{\rm Gal}(z_{\rm nrm})$ and $F_{\rm AGN}(z_{\rm nrm})$ are the flux of the galaxy and \ac{AGN} respectively. 
...
The host galaxy luminosity $L_{\rm Gal}$ as a fraction of the \ac{AGN} luminosity $L_{\rm AGN}$ decreases as the \ac{AGN} luminosity increases. 
Our quasar sample is flux limited, and so the typical \ac{AGN} luminosity increases as a function of redshift. 
...
We characterise this relation by scaling the host galaxy fraction with a power-law


We characterise the relationship between the luminosity of the AGN  and the luminosity of the host galaxy  as a power-law
...
with power-law index fixed at $\beta=0.42$ \citep{maddox06}.
\todo{Check this is what I use.} 
...
Take typical AGN luminosity as a function of redshift. 
...
We choose a reference redshift $z_{\rm nrm}$ at which the fractional contribution of the host galaxy to the total flux is determined.
If $\eta$ is the fractional contribution of the host galaxy to the total flux, then the galaxy spectrum must be scaled by the multiplicative factor $C$ in 





where the scaling factor $C$ is ter the factor by which we must multiply the unnormalised galaxy spectrum in order for Equation \ref{eq:eta} to hold true. 
